...
class RaftNode {
     ...
     * @returns {Function} The function used to calculate the node's metric.
     */
    get metricFunction() {
    ...
    /**
     * Sets the metric function.
     * @param {Function} func - The new metric function.
     * @throws {Error} If the provided value is not a function.
     */
    set metricFunction(func) {
        if (typeof func !== 'function')
            throw new Error('Invalid metric function. It must be a function.');

        this.#metricFunction = func;
        this.log('Metric function updated.');
    }
    ...
    #requestVotes() {
        const currentMetric = this.#metricFunction();
        const requesting = `${this.#currentTerm}:${this.#uuid}:${currentMetric}`;
        const options = {
            qos: 1,
            retain: false
        };
    ...
    #sendHeartbeat() {
        ...
        const currentMetric = this.#metricFunction();
        const heartbeat = `${this.#currentTerm}:${this.#uuid}:${currentMetric}`;
        const options = {
            qos: 1,
            retain: false
        };
